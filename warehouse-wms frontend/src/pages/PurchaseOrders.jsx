...
function PurchaseOrders() {
  ...
  const loadSuppliers = async () => {
    const res = await fetch(`${BASE_URL}/suppliers/all`);
    const data = await res.json();
    setSuppliers(data);
  };
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch(`${BASE_URL}/purchase-orders/create`, {
      method: "POST",
      headers: {
  ...
  const updateStatus = async (id, status) => {
    const token = localStorage.getItem("token");
    await fetch(`${BASE_URL}/purchase-orders/${id}/status`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
  ...
  const viewOrderDetails = async (orderId) => {
    const res = await fetch(`${BASE_URL}/purchase-orders/${orderId}`);
    const data = await res.json();
  ...
  const deleteOrder = async (id, poNumber) => {
    if (!window.confirm(`Are you sure you want to delete order ${poNumber}?`))
      return;
                  ...
                  <tr key={order.id}>
                    ...
                    <td>
                      ...
                      >
                        ...
                        <button
                          onClick={() => deleteOrder(order.id, order.po_number)}
                          style={{
                            background: "#ef4444",
                            color: "white",
                            border: "none",
          ...
          >
            ...
            >
              ...
              <p style={{ margin: "0.5rem 0", color: "#6b7280" }}>
                PO #: {selectedOrder.order.po_number}
              </p>
              <p style={{ margin: "0.5rem 0", color: "#6b7280" }}>
                Date:{" "}
            ...
            >
              <div>
                <p style={{ margin: "0.5rem 0", color: "#6b7280" }}>
                  <strong>Supplier:</strong> {selectedOrder.order.supplier_name}
                </p>
                <p style={{ margin: "0.5rem 0", color: "#6b7280" }}>
                  <strong>Email:</strong> {selectedOrder.order.supplier_email}
                </p>
                <p style={{ margin: "0.5rem 0", color: "#6b7280" }}>
                  <strong>Contact:</strong>{" "}
                  {selectedOrder.order.supplier_contact}
                </p>
              </div>
              <div>
                <p style={{ margin: "0.5rem 0", color: "#6b7280" }}>
                ...
                <p style={{ margin: "0.5rem 0", color: "#6b7280" }}>
                  ...
                  {selectedOrder.order.expected_delivery_date
                    ? new Date(
                        selectedOrder.order.expected_delivery_date,
                      ).toLocaleDateString()
                    : "N/A"}
