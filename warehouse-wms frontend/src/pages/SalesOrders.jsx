...
function SalesOrders() {
  ...
  const loadOrders = async () => {
    const res = await fetch("http://localhost:3000/api/sales-orders/all");
  ...
  const handleSubmit = async () => {
    ...
    try {
      const stockCheckRes = await fetch("http://localhost:3000/api/products/check-stock", {
        method: "POST",
      ...
      if (!stockCheck.valid) {
        const insufficientItems = stockCheck.issues.filter(i => i.issue === 'insufficient');
        const belowMinimumItems = stockCheck.issues.filter(i => i.issue === 'below_minimum');
        ...
        if (belowMinimumItems.length > 0) {
          const warningMsg = belowMinimumItems.map(item => 
  ...
  const viewOrderDetails = async (orderId) => {
    const res = await fetch(`http://localhost:3000/api/sales-orders/${orderId}`);
    const data = await res.json();
    ...
    <div className={styles.orders}>
      <div className={styles.header}>
        <div>
          ...
          <p className={styles.subtitle}>Manage customer orders and shipments</p>
        </div>
          ...
          <>
            <table className={styles.table}>
              ...
              <tbody>
                ...
                <tr key={order.id}>
                  <td><strong style={{cursor: 'pointer', color: '#3b82f6'}} onClick={() => viewOrderDetails(order.id)}>{order.order_number}</strong></td>
                  ...
                  <td>
                    <div style={{display: 'flex', gap: '0.5rem', alignItems: 'center'}}>
                      ...
                      <button onClick={() => deleteOrder(order.id, order.order_number)} style={{background: '#ef4444', color: 'white', border: 'none', padding: '0.4rem 0.6rem', borderRadius: '4px', cursor: 'pointer', fontSize: '0.875rem'}}>🗑️</button>
                    </div>
                  </td>
                </tr>
                ))}
          ...
          <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.formGrid}>
              ...
              <div className={styles.formGroup}>
                ...
                <input type="date" value={formData.expected_shipment_date} onChange={(e) => setFormData({...formData, expected_shipment_date: e.target.value})} />
              </div>
            ...
            <div className={styles.itemsSection}>
              ...
              {formData.items.map((item, index) => {
                ...
                const willBeBelowMinimum = selectedProduct && (availableStock - quantity < selectedProduct.minimum_stock);
                ...
                <div key={index} className={styles.itemRow}>
                  <div style={{flex: 2, position: 'relative'}}>
                    ...
                    {selectedProduct && (
                      <span style={{fontSize: '0.7rem', color: isInsufficientStock ? '#ef4444' : willBeBelowMinimum ? '#f59e0b' : '#6b7280', position: 'absolute', bottom: '-16px', left: '0', whiteSpace: 'nowrap'}}>
                        {isInsufficientStock ? `⚠️ Only ${availableStock} available!` : willBeBelowMinimum ? `⚠️ Below min` : `Avail: ${availableStock}`}
                      </span>
                    )}
                  </div>
                  <input 
                    ...
                    style={{flex: 1, borderColor: isInsufficientStock ? '#ef4444' : undefined}}
                  />
                  <input 
                    type="number" 
                    placeholder="Rate" 
                    ...
                    style={{flex: 1}}
                  />
                  <span style={{flex: 1, fontWeight: 'bold', color: '#10b981', textAlign: 'right', paddingRight: '0.5rem', minWidth: '90px', overflow: 'hidden', textOverflow: 'ellipsis'}}>₹{((parseFloat(item.quantity) || 0) * (parseFloat(item.rate) || 0)).toFixed(2)}</span>
        ...
        <div className={styles.modal} onClick={() => setShowDetailsModal(false)}>
          <div className={styles.modalContent} onClick={(e) => e.stopPropagation()} style={{maxWidth: '800px'}}>
            ...
            <div className="print-only" style={{marginBottom: '2rem', borderBottom: '2px solid #e5e7eb', paddingBottom: '1rem'}}>
              <h1 style={{margin: 0, fontSize: '1.8rem', color: '#1f2937'}}>SALES INVOICE</h1>
              <p style={{margin: '0.5rem 0', color: '#6b7280'}}>Invoice #: {selectedOrder.order.order_number}</p>
              <p style={{margin: '0.5rem 0', color: '#6b7280'}}>Date: {new Date(selectedOrder.order.order_date).toLocaleDateString()}</p>
            </div>

            <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem'}}>
              <div>
                <p style={{margin: '0.5rem 0', color: '#6b7280'}}><strong>Customer:</strong> {selectedOrder.order.customer_name}</p>
                <p style={{margin: '0.5rem 0', color: '#6b7280'}}><strong>Email:</strong> {selectedOrder.order.customer_email}</p>
                <p style={{margin: '0.5rem 0', color: '#6b7280'}}><strong>Contact:</strong> {selectedOrder.order.customer_contact}</p>
              </div>
              <div>
                ...
                <p style={{margin: '0.5rem 0', color: '#6b7280'}}><strong>Expected Shipment:</strong> {selectedOrder.order.expected_shipment_date ? new Date(selectedOrder.order.expected_shipment_date).toLocaleDateString() : 'N/A'}</p>
